...
void print_set(char *nam, full_addmap_t &s, int max_nprint = MAX_NPRINT, ostream &ostr = cerr);
void print_set(char *nam, cfset_t &s, int max_nprint = MAX_NPRINT, ostream &ostr = cerr);
...
void _solve() {
    ...
    while(!TARGETS.empty()) {
        ...
        if(!TARGETS.empty())
            if(!USE_AC1) heur_synth_best();
            else ac1_decompose_cheapest();
